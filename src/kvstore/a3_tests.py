...
class TestSuite:
    ...
    def print_results(self):
        ...
        for r in self.results:
            ...
            print(
                f"{r.name:<35} [{status}] {r.score:.1f}/{r.max_points:.1f} — {r.details}"
            )
...
def wait_for_leader_election(num_servers, timeout_seconds=15):
    ...
    while time.time() - start_time < election_timeout:
        ...
        print(
            f"  Election check: {len(leaders)} leaders found, states: {server_states}"
        )
...
def test_config_file():
    ...
    if os.path.exists(filename_config):
        ...
        return TestResult(
            test_name, 0, test_max_points, f"Config file {filename_config} missing."
        )
...
def test_server_startup():
    ...
    if len(responsive_servers) != 3:
        return TestResult(
            ...
            5,
            test_max_points,
            f"Only {len(responsive_servers)}/3 servers responsive",
        )
    ...
    if state_working == 3:
        ...
        return TestResult(
            ...
            10,
            test_max_points,
            f"Only {state_working}/3 servers respond to GetState",
        )
...
def test_leader_election_basic():
    ...
    leaders = [
        sid for sid, state in server_states.items() if state.get("is_leader", False)
    ]
    if len(leaders) != 1:
        return TestResult(
            test_name,
    ...
    if len(set(terms)) > 1:
        return TestResult(
            test_name, 15, test_max_points, f"Servers have different terms: {terms}"
        )
...
def test_leader_election_larger_cluster():
    ...
    if len(responsive_servers) < 5:
        return TestResult(
            ...
            5,
            test_max_points,
            f"Only {len(responsive_servers)}/5 servers responsive",
        )
...
def test_request_vote_rpc():
    ...
    if not success:
        return TestResult(test_name, 0, test_max_points, "RequestVote RPC call failed")
    ...
    success2, response_term2, vote_granted2 = call_request_vote(
        0, 1, 2
    )  # Different candidate
    if not success2:
        return TestResult(
    ...
    print(
        f"Second RequestVote response: term={response_term2}, voteGranted={vote_granted2}"
    )
    ...
    print(
        f"Higher term RequestVote response: term={response_term3}, voteGranted={vote_granted3}"
    )
...
def test_election_timeout():
    ...
    if not success:
        return TestResult(test_name, 0, test_max_points, "No initial leader elected")
    ...
    final_success, final_leader, final_term, final_states = wait_for_leader_election(
        3, timeout_seconds=5
    )
    ...
    if final_term < leader_term:
        return TestResult(
            ...
            7,
            test_max_points,
            f"Term decreased: {leader_term} -> {final_term}",
        )
...
def test_state_transitions():
    ...
    if not success:
        return TestResult(
            ...
            1,
            test_max_points,
            "No leader elected for state transitions test",
        )
