...
def test_server_startup():
    ...
    if not ping_server(0):
        return TestResult(
            ...
            5,
            test_max_points,
            "StartRaft succeeded but server 0 not responding to ping",
        )
...
def test_server_basic_kv():
    ...
    if not success:
        return TestResult(test_name, 0, test_max_points, f"Server GET failed: {value}")
    ...
    if value != "":
        return TestResult(
            ...
            5,
            test_max_points,
            f"GET missing key should return empty string, got: '{value}'",
        )
    ...
    if not server_put(0, test_key, test_value):
        return TestResult(test_name, 10, test_max_points, "Server PUT operation failed")
...
def test_frontend_kv_operations():
    ...
    if value != "":
        return TestResult(
            ...
            5,
            test_max_points,
            f"Frontend GET missing key should return empty string, got: '{value}'",
        )
    ...
    for i, (key, value) in enumerate(zip(workflow_keys, workflow_values)):
        ...
        if not success:
            return TestResult(
                ...
                i * 3,
                test_max_points,
                f"Workflow PUT failed for {key}: {error}",
            )
    ...
    for i, (key, expected_value) in enumerate(zip(workflow_keys, workflow_values)):
        ...
        if not success:
            ...
            return TestResult(
                ...
                points,
                test_max_points,
                f"Workflow GET failed for {key}: {error}",
            )
