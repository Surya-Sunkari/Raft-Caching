...
class ExecutionConfig:
    ...
    def load_config(self, config_file):
        ...
        default_config = {
            ...
            "persistence": {"state_dir": "raft_state"},
        }
...
class TestSuite:
    ...
    def print_results(self):
        ...
        for r in self.results:
            ...
            print(
                f"{r.name:<50} [{status}] {r.score:.1f}/{r.max_points:.1f} — {r.details}"
            )
...
def test_basic_persistence():
    ...
    if not exists:
        return TestResult(
            test_name, 5, test_max_points, "No persistent state files created"
        )
...
def test_full_cluster_restart():
    ...
    for key, expected_value in test_data.items():
        ...
        if not success or value != expected_value:
            return TestResult(
                test_name, 7, test_max_points, f"Data not committed before crash: {key}"
            )
    ...
    if not exists:
        return TestResult(
            test_name, 10, test_max_points, "No persistent state after cluster crash"
        )
    ...
    for i in range(5):
        ...
        if not success:
            return TestResult(
                test_name, 12, test_max_points, f"Failed to restart server {i}: {error}"
            )
        time.sleep(1)
    ...
    if not success:
        return TestResult(
            test_name, 15, test_max_points, "No leader elected after cluster restart"
        )
    ...
    for key, expected_value in test_data.items():
        ...
        if not success or value != expected_value:
            all_correct = False
            print(
                f"  {key}: LOST or CORRUPTED - expected '{expected_value}', got '{value}'"
            )
        else:
    ...
    if all_correct:
        ...
        return TestResult(
            test_name, 17, test_max_points, "Some data lost after cluster restart"
        )
...
def test_network_partition_majority():
    ...
    if not server_put(leader_id, test_key_before, test_value_before):
        return TestResult(
            test_name, 3, test_max_points, "Failed to write data before partition"
        )
    ...
    if not success:
        return TestResult(
            test_name, 7, test_max_points, "Majority partition failed to elect leader"
        )
    ...
    if not server_put(maj_leader, test_key_during, test_value_during):
        return TestResult(
            test_name, 10, test_max_points, "Majority partition cannot accept writes"
        )
    ...
    if not success1 or val1 != test_value_before:
        return TestResult(
            test_name, 12, test_max_points, "Old data lost in majority partition"
        )

    if not success2 or val2 != test_value_during:
        return TestResult(
            test_name, 12, test_max_points, "New data not written in majority partition"
        )
...
def test_network_partition_minority():
    ...
    try:
        ...
        print(
            f"Creating network partition: killing majority servers {majority_servers}"
        )
        ...
        if not write_attempted:
            return TestResult(
                test_name, 0, test_max_points, "No minority servers responding"
            )
...
def test_partition_healing():
    ...
    for i in range(3):
        ...
        if not server_put(leader_id, key, value):
            return TestResult(
                test_name, 3, test_max_points, f"Failed to write pre-partition data {i}"
            )
        time.sleep(0.3)
    ...
    if not success:
        return TestResult(
            test_name, 5, test_max_points, "Majority failed to elect leader"
        )
    ...
    for server_id in range(5):
        ...
        for key, expected_value in all_data.items():
            ...
            if not success or value != expected_value:
                consistent = False
                print(
                    f"  Server {server_id} - {key}: expected '{expected_value}', got '{value}'"
                )
            else:
    ...
    if consistent:
        ...
        return TestResult(
            test_name, 15, test_max_points, "Partition healed but inconsistency remains"
        )
...
def test_persistence_with_leader_changes():
    ...
    for round in range(3):
        ...
        if not success:
            return TestResult(
                test_name, round * 3, test_max_points, f"No leader in round {round}"
            )
    ...
    if all_correct:
        ...
        return TestResult(
            test_name, 8, test_max_points, "Some data lost during leader changes"
        )
...
def test_start_raft_clears_state():
    ...
    if not success:
        return TestResult(
            test_name, 0, test_max_points, f"Failed to start first cluster: {error}"
        )
    ...
    if not success or value != old_value:
        return TestResult(
            test_name, 4, test_max_points, "Test data not written correctly"
        )
    ...
    if not success:
        return TestResult(
            test_name, 6, test_max_points, f"Failed to start new cluster: {error}"
        )
    ...
    return TestResult(test_name, 8, test_max_points, "Unclear if state was cleared")
