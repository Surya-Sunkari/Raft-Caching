...
```bash
cd src/kvstore
python benchmark.py                       # Unit mode: drives cache.py directly (fast, 5 policies x 3 workloads x 3 capacities)
...
- **`cache.py`** — Pluggable cache eviction policies (random, fifo, lru, lfu, slru). Abstract base `CachePolicy` with `get/put/invalidate/clear` interface. Factory: `create_cache(policy_name, capacity)`.
